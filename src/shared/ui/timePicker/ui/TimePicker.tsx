...
export const TimePicker: FC = () => {
  ...
  return (
    <div className="relative w-40" ref={timePickerRef}>
      <input
        type="text"
        readOnly
        className="w-full p-2 border rounded-md cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              {hoursArray.map((hour) => (
                <div
                  ...
                  className={`p-2 text-center cursor-pointer rounded-md transition-colors
                    ${hour === hours ? "bg-blue-500 text-white" : "hover:bg-gray-100"}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    setHours(hour);
                  }}
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-gray-100">
              {minutesArray.map((minute) => (
                <div
                  key={minute}
                  className={`p-2 text-center cursor-pointer rounded-md transition-colors
                    ${minute === minutes ? "bg-blue-500 text-white" : "hover:bg-gray-100"}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    setMinutes(minute);
                  }}
